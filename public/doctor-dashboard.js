...
function doctorApi(path, options = {}) {
  ...
  return fetch(path, {
    ...
    console.log(`📥 Response ${path}:`, res.status, data);
    if (!res.ok) throw new Error(data.error || data.message || `Request failed (${res.status}) while loading ${path}`);
    return data;
  });
}
...
async function initDoctorDashboard() {
  console.log("🔵 ========== INIT DOCTOR DASHBOARD ==========");
  ...
  try {
    console.log("🔵 Step 1: Fetching /api/auth/me...");
    const me = await doctorApi("/api/auth/me");
    console.log("✅ /api/auth/me response:", me);
    
    if (!me.account || me.account.role !== "doctor") {
      console.log("❌ Not a doctor role:", me.account?.role);
      window.location.href = "/";
      return;
    }
    doctorState.account = me.account;
    console.log("✅ Doctor loaded:", doctorState.account.name);
    
    console.log("🔵 Step 2: Setting online status...");
    ...
    console.log("🔵 Step 3: Rendering shell...");
    renderShell();
    
    console.log("🔵 Step 4: Loading patients data...");
    await loadPatients();
    console.log("✅ Dashboard loaded successfully!");
    
  } catch (err) {
    console.error("❌ ERROR in initDoctorDashboard:", err);
    ...
    if (doctorApp) {
      doctorApp.innerHTML = `
        <div style="padding: 40px; max-width: 800px; margin: 0 auto;">
          <div style="background: #ffebee; border: 1px solid #ef9a9a; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
            <h2 style="color: #c62828; margin: 0 0 10px 0;">❌ Dashboard Error</h2>
            <p style="margin: 0 0 10px 0;"><strong>Error:</strong> ${safe(err.message)}</p>
            <details style="margin-top: 10px;">
              <summary style="cursor: pointer; color: #666;">View Details</summary>
              <pre style="background: #f5f5f5; padding: 10px; overflow-x: auto; font-size: 12px; margin-top: 10px;">${safe(err.stack || "No stack trace")}</pre>
            </details>
          </div>
          <button onclick="logoutDoctor()" style="background: #e91e63; color: white; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer;">Go Back to Login</button>
        </div>
      `;
    } else {
      setTimeout(() => logoutDoctor(), 2000);
    }
  }
}
...
function renderShell() {
  ...
  if (logoutBtn) logoutBtn.addEventListener("click", logoutDoctor);
...
async function loadPatients() {
  console.log("🔵 loadPatients: Starting data fetch...");
  
  try {
    const promises = [];
    
    console.log("🔵 Fetching /api/doctor/patients...");
    promises.push(doctorApi("/api/doctor/patients").catch(err => {
      console.error("❌ Failed to fetch patients:", err);
      return { summary: {}, patients: [], alerts: [], conditions: [] };
    }));
    
    console.log("🔵 Fetching /api/chat/doctor/chats...");
    promises.push(doctorApi("/api/chat/doctor/chats").catch(err => {
      console.error("❌ Failed to fetch chats:", err);
      return { chats: [] };
    }));
    
    console.log(`🔵 Fetching /api/appointments/doctor/${doctorState.account?.id}...`);
    promises.push(doctorApi(`/api/appointments/doctor/${doctorState.account?.id}`).catch(err => {
      console.error("❌ Failed to fetch appointments:", err);
      return { appointments: [] };
    }));
    
    console.log("🔵 Fetching /api/doctor/insights...");
    promises.push(doctorApi("/api/doctor/insights").catch(err => {
      console.error("❌ Failed to fetch insights:", err);
      return { highRiskPatients: [], highPainAlerts: [], frequentSymptoms: [] };
    }));
    
    const [data, chatData, appointmentData, doctorInsights] = await Promise.all(promises);
    
    console.log("✅ Data received:", {
      patients: data.patients?.length || 0,
      chats: chatData.chats?.length || 0,
      appointments: appointmentData.appointments?.length || 0,
      insights: doctorInsights.highRiskPatients?.length || 0
    });
    
    doctorState.summary = data.summary || { totalPatients: 0, activePatients: 0, highRiskPatients: 0, newPatients: 0 };
    ...
    doctorState.doctorInsights = doctorInsights || { highRiskPatients: [], highPainAlerts: [], frequentSymptoms: [] };
    ...
    if (contentDiv) {
      contentDiv.innerHTML = `
        <div style="background: #ffebee; border: 1px solid #ef9a9a; border-radius: 8px; padding: 20px; margin: 20px;">
...
function renderDashboardContent() {
  ...
  contentDiv.innerHTML = `
    <section id="overview" class="doctor-summary">
      <article class="doctor-card metric-card"><span>Total Patients</span><strong>${summary.totalPatients || 0}</strong></article>
      <article class="doctor-card metric-card"><span>Active Patients</span><strong>${summary.activePatients || 0}</strong></article>
      <article class="doctor-card metric-card high"><span>High Risk Patients</span><strong>${summary.highRiskPatients || 0}</strong></article>
      <article class="doctor-card metric-card"><span>New Patients</span><strong>${summary.newPatients || 0}</strong></article>
    </section>
    ...
    <section id="alerts" class="doctor-card alerts-panel">
      ...
      <div class="alerts-list">
        ${doctorState.alerts.length ? doctorState.alerts.map((alert) => `
          <button class="alert-item ${(alert.riskLevel || 'low').toLowerCase()}" data-patient="${alert.patientId}">
      ...
      <div class="appointment-list">
        ${todayAppointments().length ? todayAppointments().map((item) => `
          <article class="appointment-row">
            ...
            <button class="btn secondary" data-cancel-appt="${safe(item.id || item.appointmentId)}">Cancel</button>
          </article>
  ...
  ["patientSearch", "conditionFilter", "riskFilter"].forEach((id) => {
    ...
    if (el) el.addEventListener("input", renderPatientRows);
  });
  ...
  // FIXED: Cancel appointment endpoint
  document.querySelectorAll("[data-cancel-appt]").forEach((button) => {
    button.addEventListener("click", async () => {
      const appointmentId = button.dataset.cancelAppt;
      console.log("🔵 Cancelling appointment:", appointmentId);
      await doctorApi(`/api/appointments/${appointmentId}/cancel`, {
        method: "PATCH",
...
function renderDoctorChatList() {
  ...
  list.innerHTML = doctorState.chats.length ? doctorState.chats.map((chat) => `
    <button class="doctor-chat-contact ${doctorState.activeChat?.chatId === chat.chatId ? "active" : ""}" data-open-chat="${chat.userId}">
      ...
      <span>
        <strong>${safe(chat.patient?.name || "Unknown")}</strong>
...
function renderDoctorChatWindow() {
  ...
  box.innerHTML = `
    <header class="chat-header compact">
      <div>
        <h3>${safe(chat.patient?.name || "Patient")}</h3>
  ...
  const replyForm = document.getElementById("doctorReplyForm");
  if (replyForm) replyForm.addEventListener("submit", sendDoctorReply);
  
  const replyInput = document.getElementById("doctorReplyInput");
  if (replyInput) replyInput.addEventListener("input", sendDoctorTyping);
}
...
function renderPatientRows() {
  ...
  tbody.innerHTML = rows.length ? rows.map((patient) => `
    <tr>
      <td><strong>${safe(patient.name)}</strong>电子
      <td>${safe(patient.email)}电子
      <td>${safe(patient.condition)}电子
      <td>${formatDate(patient.lastActivity)}电子
      <td><span class="${riskClass(patient.riskLevel)}">${safe(patient.riskLevel)}</span>电子
      <td><button class="btn secondary" data-view-patient="${patient.id}">View Details</button>电子
    </tr>
  `).join("") : `
    <tr><td colspan="6" class="empty-row" style="text-align: center;">No matching patients.</td></tr>
  `;
...
async function openPatient(patientId) {
  const [data, sharedData] = await Promise.all([
    ...
    doctorApi("/api/symptoms/shared").catch(() => ({ symptoms: [] }))
  ]);
...
function renderPatientModal(patient) {
  ...
  modal.innerHTML = `
    <section class="patient-modal">
      ...
      <div class="patient-modal-head ${(patient.risk?.level || 'low').toLowerCase()}">
        <div>
          <p class="doctor-kicker">Patient Details</p>
          <h2>${safe(patient.name)}</h2>
          <p>${safe(patient.email)} &middot; ${safe(patient.profileType)}</p>
        </div>
        <span class="${riskClass(patient.risk?.level || 'low')}">${safe(patient.risk?.level || 'Low')} Risk</span>
      </div>
      ...
      <div class="detail-grid two">
        <article class="detail-panel">
          ...
          <div class="symptom-history compact">
            ${patient.symptomHistory?.length ? patient.symptomHistory.slice(0, 10).map((entry) => `
      ...
      <div class="detail-grid two">
        <article class="detail-panel">
          <h3>Activity Logs</h3>
          <div class="timeline">
            ${patient.activityLogs?.length ? patient.activityLogs.slice().reverse().slice(0, 10).map((log) => `
      ...
      <div class="detail-grid two">
        <article class="detail-panel">
          <h3>Messages</h3>
          ${patient.messages?.length ? patient.messages.slice().reverse().slice(0, 10).map((item) => `<p>${safe(item.message)}<br><small>${formatDate(item.createdAt)}</small></p>`).join("") : `<p class="muted">No messages sent.</p>`}
        </article>
        <article class="detail-panel">
          <h3>Recommendations</h3>
          ${patient.recommendations?.length ? patient.recommendations.slice().reverse().slice(0, 10).map((item) => `<p>${safe(item.recommendation)}${item.followUpDate ? `<br><strong>Follow-up:</strong> ${safe(item.followUpDate)}` : ""}<br><small>${formatDate(item.createdAt)}</small></p>`).join("") : `<p class="muted">No recommendations added.</p>`}
        </article>
      </div>
    </section>
  `;
  ...
  if (messageForm) messageForm.addEventListener("submit", saveMessage);
  
  const recommendationForm = document.getElementById("recommendationForm");
  if (recommendationForm) recommendationForm.addEventListener("submit", saveRecommendation);
}
...
async function saveMessage(event) {
  event.preventDefault();
  const messageInput = document.getElementById("doctorMessage");
...
async function saveRecommendation(event) {
  event.preventDefault();
  const recommendationInput = document.getElementById("doctorRecommendation");
